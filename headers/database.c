    ...
    while(fscanf(f," %c",&type)==1){
        if(type=='P'){
            ...
            fscanf(f," %49s %49s %19s %99s %19s %19s %9s %f %d",
                   s->name,s->surname,s->personalID,s->email,s->phone,s->birthDate,s->gender,&s->averageGrade,&s->parentCount);
            for(int i=0;i<s->parentCount;i++) fscanf(f," %19s", s->parentPersonalIDs[i]);
            (*studentCount)++;
        }
    }
    fclose(f);
