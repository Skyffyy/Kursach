...
void loadDatabase(struct Student students[], struct Parent parents[], int *studentCount, int *parentCount){
    ...
    if(!f){
        ...
        if(!f){ printf("Cannot create database file.\n"); *studentCount=0; *parentCount=0; return;}
        fclose(f);
        *studentCount = 0;
        *parentCount = 0;
        return;
    ...
    *parentCount = 0;
    ...
    while(fscanf(f," %c",&type)==1){
        if(type=='P'){
            ...
            s->parentCount = 0;
            fscanf(f,"%49s %49s %19s %99s %19s %19s %9s %f %d %d",
                   s->name, s->surname, s->personalID, s->email, s->phone,
                   s->birthDate, s->gender, &s->averageGrade, &s->birthYear, &s->parentCount);
...
void saveDatabase(struct Student students[], struct Parent parents[], int studentCount, int parentCount){
    ...
    if(!f){ printf("Cannot save database file.\n"); return; }
    ...
    for(int i=0;i<studentCount;i++){
        ...
        fprintf(f,"S %s %s %s %s %s %s %s %.2f %d %d",
                s.name, s.surname, s.personalID, s.email, s.phone,
                s.birthDate, s.gender, s.averageGrade, s.birthYear, s.parentCount);
